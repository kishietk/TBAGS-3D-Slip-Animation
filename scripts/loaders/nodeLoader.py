...
def load_nodes(path: str = NODE_CSV, filter_ids: set = None, mode: str = "normal") -> Dict[int, NodeData]:
    """
                ...
                if m_node:
                    ...
                    if mode == "copy":
                        if filter_ids and nid not in filter_ids:
                            log.debug(f"[{path}]  ID {nid} at row {row_idx}: not in COPY_NODE_IDS")
                            continue
                    else:
                        if (filter_ids and nid not in filter_ids):
                            log.debug(f"[{path}]  ID {nid} at row {row_idx}: not in VALID_NODE_IDS")
                            continue
