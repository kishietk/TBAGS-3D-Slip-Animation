...
def on_frame(
    ...
    #     if arm_obj.type == 'ARMATURE' and "model" in arm_obj.name.lower():
    #         pose_bones = arm_obj.pose.bones
    #         for nid, anim_per_frame in sandbag_anim_data.items():
    #             bone_name = str(nid)
    #             if bone_name not in pose_bones:
    #                 continue
    #             pose_bone = pose_bones[bone_name]
    #             base_pos = base_sandbag_pos[nid] if (base_sandbag_pos and nid in base_sandbag_pos) else Vector((0,0,0))
    #             for frame, disp in anim_per_frame.items():
    #                 pose_bone.location = base_pos + disp
    #                 for i in range(3):
    ...
    for obj in panel_objs:
        try:
            ...
            verts = [
                all_node_objs[a].matrix_world.translation,
                all_node_objs[b].matrix_world.translation,
                all_node_objs[d].matrix_world.translation,
                all_node_objs[c].matrix_world.translation,
            ]
            mesh = obj.data
            mesh.clear_geometry()
            bm = bmesh.new()
    ...
    for obj, a, b in member_objs:
        try:
            if obj is None:
                continue
            p1 = all_node_objs[a].matrix_world.translation
            p2 = all_node_objs[b].matrix_world.translation
