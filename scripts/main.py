...
def main() -> None:
    ...
    try:
        ...
        log.info(f"base_node_pos IDs: {list(base_node_pos.keys())}")
        log.info(f"base_sandbag_pos IDs: {list(base_sandbag_pos.keys())}")
        log.info(f"sandbag_anim_data IDs: {list(sandbag_anim_data.keys())}")
        ...
        # サンドバック　アニメーション作成
        from sandbag_move import bake_sandbag_bone_animation
        bake_sandbag_bone_animation(
            sandbag_anim_data=sandbag_anim_data,
        )

        # サンドバックをコピー配置
        from sandbag_copy import copy_selected_nodes
        copy_selected_nodes()

        # # キーフレームごとに統合する
        # from animators.keyframe import bake_combined_sandbag_animation
        # bake_combined_sandbag_animation()
