...
root/scripts/直下
root/scripts/loaders/直下
root/scripts/cores/直下
root/scripts/builders/直下
root/scripts/utils/直下
root/data/直下
animation.csvはもっと巨大なデータですので、「...」で行を大幅に削っています。ここまでで、使用するデータは共有しました。

次はblenderのボーンをつかった柱の表現を行いたい。
...
blenderのボーンをつかった柱の表現を行いたい。
いまだと、ノードで分割されていて、柱がポキポキ折れるようなアニメ―ションになっているから、このあたりをリアルな曲がり方にしたいと思っている。
柱を1本化し、ノードの変位アニメーションをボーンに利用してアーマチュア連動アニメーション化
└ 構造部材が物理的に“折れずに連続して変形”するよう、ボーンに追従

現在は、これの移行作業の途中でうまくいかず困っています。
column_grouper.py
ここで、柱グループを作成して、
columns_armature.py
ここでボーン入り柱（4ノード分の長さ）を生成したいのですが、うまくできません。